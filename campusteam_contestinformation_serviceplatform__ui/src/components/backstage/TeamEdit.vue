<template>
    <div class="backteamLabel-view3 ">
        <div class="backteamLabel-model3 ">
            ...
            <div class="form-item ">
                <div class="item-title" style="color:#a7a7a7;">
                    邀&nbsp;请&nbsp;码
                </div>
                <div class="item-content" style="color:#a7a7a7;">
                    {{ team.teamInviteCode }}
                </div>
            </div>
            ...
            <!-- 确认按钮 -->
            <div class="form-item ">
                <div class="joined-is-open">开放招新</div>
                <el-switch class="switch2" v-model="team.teamStatus" active-color="#13ce66" inactive-color="#909399">
                </el-switch>
                <div class="do-add-team-btn ts02s" @click="doEditTeam()">
                    确认并提交
                </div>
                <div class="do-del-team-btn ts02s" @click="doDeleleTeam()">
                    解散
                </div>
            </div>
            <div class="form-item"></div>
...
export default {
    ...
    methods: {
        ...mapActions('team', ['updateTeamById']),
        ...mapActions('team', ['deleteTeamById']),
        /**
         * 选择logoCss
         * @param {*} index 
         */
        ...
        async doEditTeam() {
            ...
            // 提交数据库
            const { code, message, data } = await this.updateTeamById(team)
            if (code === 200) {
                // 跳转详情页
         ...
         */
        async doDeleleTeam() {
            this.$confirm(this.team.teamName, '确定解散队伍？', {
                confirmButtonText: '确定',
                cancelButtonText: '取消',
                type: 'warning'
            }).then(async () => {
                const { code, message, data } = await this.deleteTeamById(this.team)
                if (code === 200) {
                    ElementUI.Message.info('队伍已解散')
                    setTimeout(() => {
                        this.back()
                        setTimeout(() => {
                            location.reload()
        ...
        ...mapActions('team', ['selectTeamById']),
        /* 初始化 */
        async init() {
            const { code, message, data } = await this.selectTeamById(this.teamId)
            if (code === 200) {
                this.team = { ...data }
                this.team.teamCreateTime = this.$moment(this.team.teamCreateTime).format('YYYY-MM-DD HH:mm:ss')
                this.avators.forEach((item, index) => {
                    if (item == this.team.teamLogo) {
                        this.chooseLogo(index)
                    }
                })
            }
        }
